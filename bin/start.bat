...
if %gpu_available% equ 0 (
    ...
    docker.exe compose --env-file ..\project.env -f ..\compose.yml -f ..\compose-gpu.yml up --no-recreate --no-build 
) else (
    echo NVIDIA GPU is not available
    docker.exe compose --env-file ..\project.env -f ..\compose.yml up --no-recreate --no-build 
)
