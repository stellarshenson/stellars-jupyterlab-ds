...
125. **Task - Disable AppStream and install Playwright Chromium**: Mirror sync resilience plus system-wide browser<br>
    **Result**: Build was failing on `apt update` with `File has unexpected size` errors fetching `dep11/Components-amd64.yml.gz` files (Ubuntu mirror sync race). Replaced `apt update` line in `Dockerfile.jupyterlab` (line 136) with a heredoc RUN block that writes `/etc/apt/apt.conf.d/99no-appstream` setting `Acquire::IndexTargets::deb::DEP-11 "false"`, `Acquire::IndexTargets::deb::DEP-11-icons "false"`, and `Acquire::IndexTargets::deb-src::DEP-11 "false"` before the first apt update. The config persists for all subsequent apt operations including the second `apt update && apt upgrade` on line 142 and the apt-get calls run internally by `playwright install --with-deps`. Added Playwright Chromium installation system-wide at `/opt/ms-playwright`: added `playwright` to pip section of `environment_base_jupyterlab.yml`, then added new RUN block in `Dockerfile.jupyterlab` after the `jupyter_app_launcher` chmod block (still inside existing `USER root` context, no extra USER switch) that sets `ENV PLAYWRIGHT_BROWSERS_PATH=/opt/ms-playwright`, runs `playwright install --with-deps chromium` via `${CONDA_CMD} run --name ${CONDA_DEFAULT_ENV}`, and chmods the path world-readable. Required by `jupyterlab_export_markdown_extension` (rasterises SVGs for DOCX/PDF export), Playwright MCP server (lets AI assistants drive a browser), and agentic AI workflows that operate user webapps (Streamlit, Shiny, Gradio, Chainlit, MLFlow) running behind the JupyterLab proxy
