...
121. **Task - Add timezone startup script**: Container timezone from JUPYTERLAB_TIMEZONE env var<br>
    **Result**: Created `07_set_timezone.sh` in `start-platform.d/` that sets container timezone via symlink to `/usr/share/zoneinfo` when `JUPYTERLAB_TIMEZONE` is set (IANA format, e.g. `Europe/Warsaw`). Skips silently when empty - container stays UTC. Validates that the zoneinfo file exists before applying. Added `JUPYTERLAB_TIMEZONE=""` default to `Dockerfile.jupyterlab` alongside other JUPYTERLAB env vars, empty value in `compose.yml` for site-specific override, and documented in README under JupyterLab Settings
