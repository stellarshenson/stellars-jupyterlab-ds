...
119. **Task - Fix full build failures**: BuildKit DNS and docker-buildx compilation<br>
    **Result**: Full rebuilds (`make build`) failed with two issues. First, BuildKit DNS resolution failures on WSL2 - `CondaHTTPError: HTTP 000 CONNECTION FAILED` for conda-forge during `docker compose build`. Fixed by adding `network: host` to compose.yml build section, which makes BuildKit use host networking instead of its own DNS resolver. Direct `docker build --network=host` was proven to work first, then compose.yml configuration confirmed working with Docker Desktop 4.60. Second, docker-buildx builder step (Dockerfile line 96) failed because `make build` in the buildx repo now calls `docker buildx bake binaries` which requires Docker inside the build container. Replaced with direct Go compilation: `CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags "-s -w" -o ... ./cmd/buildx`, matching the pattern already used for docker-mcp-gateway. This step had always been cached in `make rebuild` (target-only) so the breakage was only exposed during full rebuilds
