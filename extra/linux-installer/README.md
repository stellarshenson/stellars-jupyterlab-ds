...
Self-extracting all-in-one shell installer - the user downloads a single `.sh`, no repository checkout needed. The deployment files (`compose.yml`, `compose-gpu.yml`, `.env.default`, `start.sh`, `stop.sh`, `LICENSE`) are embedded as a gzipped tar payload; the platform image is pulled from Docker Hub on first start.
...
Output: `dist/stellars-jupyterlab-ds-setup-{version}.sh` in the repo-root `dist/` folder - version read from the root `pyproject.toml`, older installers removed from `dist/` first (`dist/` is gitignored). `make build` at the repo root builds it automatically after the docker image.
