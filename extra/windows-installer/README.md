...
- **Password** - asked during install, saved to `.env` as `JUPYTERLAB_SERVER_TOKEN`; the JupyterLab login page asks for it, it is never placed in the URL and can be changed later in `.env` (restart to apply)
...
Output: `dist/stellars-jupyterlab-ds-setup-{version}.exe` in the repo-root `dist/` folder - version read from the root `pyproject.toml`, older installers removed from `dist/` first (`dist/` is gitignored).
