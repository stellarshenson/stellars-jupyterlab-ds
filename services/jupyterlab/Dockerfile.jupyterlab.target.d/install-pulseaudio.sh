...
# pulseaudio-utils, sox and libsox-fmt-pulse and writes the default-server line
# to /etc/pulse/client.conf. It does NOT start the daemon - that happens at
# boot via start-platform.d/15_start_pulseaudio.sh.
#
...
echo "installing PulseAudio + SoX voice stack via jupyterlab_voice_capture_extension"
