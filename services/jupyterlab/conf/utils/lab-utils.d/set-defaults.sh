#!/bin/bash
## Set default configurations (AWS profile, conda env, shell)

set -e

# Get the directory where this script is located
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPTS_DIR="$SCRIPT_DIR/set-defaults.d"
...
TEMPFILE=$(mktemp)
dialog --clear --title "Set Defaults" \
	--menu "Select setting to configure:" \
	20 80 10 \
	"${MENU_OPTIONS[@]}" \
	2>$TEMPFILE >/dev/tty || {
