...
## terminals go through the launcher, which resolves the user's Default Shell
## (env store, then JUPYTERLAB_TERMINAL_SHELL env, then /bin/bash) at SPAWN
## time and execs it as a login shell - a Settings > Default Shell selection
## applies to the next terminal without a server restart
c.ServerApp.terminado_settings = { "shell_command": ["/opt/utils/launch-shell.sh"] }
...
c.ServerApp.tornado_settings = {
    'headers': {
        'Content-Security-Policy': "frame-ancestors 'self'; default-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob:; img-src * data: blob: 'unsafe-inline' 'unsafe-eval';"
    }
}

## rmonitor terminal-sizing fix lives in rmonitor_rewrite.py next to this file
import sys
sys.path.insert(0, os.path.dirname(__file__))
from rmonitor_rewrite import rewrite_rmonitor_response
...
c.ServerProxy.servers = {
    "rmonitor": {
        ...
        "rewrite_response": rewrite_rmonitor_response,
        "launcher_entry": {
            "enabled": True,
            "category": "Services",
